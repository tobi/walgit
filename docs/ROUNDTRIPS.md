...
| WAL rewind (`snapshot` op / `wal materialize --at-seq`; operator-driven, never a hot path) | 1 cond GET (refs sync, validates `at_seq` against the head) → checkpoint + its refs (2 GETs when one is at or before the cut) → the log segments through the cut → per pack not already local: pack ∥ idx ∥ advertised side-files (`fetch_pack_into`, striped). A sequence already materialized here answers from the local marker: **0 requests after the freshness GET** | 1 + 2 + segments + 3–5 per missing pack | `snapshot.rs::materialize_inner`, `handle.rs::fetch_pack_into` |
