...
#[test]
fn sas_credentials_require_tls_outside_loopback() {
    let mut cfg = StoreConfig {
        bucket: "container".into(),
        ..Default::default()
    };
    for host in ["localhost", "127.0.0.1", "[::1]"] {
        cfg.azure.endpoint = format!("http://{host}:10000/account");
        assert!(container_url(&cfg, Some("sig=synthetic")).is_ok());
    }
    cfg.azure.endpoint = "http://storage.example/account".into();
    assert!(container_url(&cfg, Some("sig=synthetic")).is_err());
}

#[tokio::test]
async fn undersized_small_body_declarations_stop_before_reading_more_input() {
    let (mut store, calls) = fixture(
        |_| async { panic!("invalid input must not reach storage") },
        None,
    );
    store.multipart_threshold = 100;
    let reads = Arc::new(AtomicUsize::new(0));
    let count = reads.clone();
