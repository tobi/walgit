...
// Mirrors the `[wal]` section of walgit.toml: its flags are that section's keys.
...
// Mirrors the `[bundles]` section of walgit.toml: its flags are that section's keys.
#[allow(clippy::struct_excessive_bools)]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields, default)]
...
// Mirrors the `[git]` section of walgit.toml: its flags are that section's keys.
#[allow(clippy::struct_excessive_bools)]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(deny_unknown_fields, default)]
        ...
        for (k, v) in vars {
            ...
            let bad = {
                fn set(
                    ...
                    // The loop above drops a variable whose path is empty, and the
                    // recursive call always passes the tail of a path of two or
                    // more, so an empty path here names a key nothing can set
                    // rather than an index into nothing.
                    let Some((head, rest)) = path.split_first() else {
                        return Err("names no key after WALGIT__".to_string());
                    };
                    if rest.is_empty() {
